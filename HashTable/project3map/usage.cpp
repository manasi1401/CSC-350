...
int checkAndOpen(ifstream &fin, ofstream &foutWrd, ofstream &foutCsv, char  *input, string &inputFileName)
{
  ...
  else
  {
    ...
    filefile = new char[fileNametemp.size()+1];
    strcpy(filefile, fileNametemp.c_str());
    inputFileName = strtok(filefile, "");
	  outputNameWrd = strtok(filefile, ".");
    cout<<inputFileName<<endl;
    
    inputFileName =intputtest.substr(0, slashpos+1) +inputFileName;
